...
def countries_detail(request,pk):
    ...
    if request.method == 'GET':
        ...
        if countries_serializer.is_valid():
            countries_serializer.save()
            return JsonResponse(countries_serializer.data)
        return JsonResponse(countries_serializer.error, status.HTTP_400_BAD_REQUEST)

    elif request.method == 'DELETE':
        countries.delete()
        return JsonResponse({'message':'The country was deleted!'},status=status.HTTP_204_NO_CONTENT)